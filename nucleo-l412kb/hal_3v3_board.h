...
#define DEBOUNCE_MS 8  // LED blinking period in millis
...
#define COL6_PIN PIN('A', 0)  // first key
#define COL5_PIN PIN('A', 1)  
#define COL4_PIN PIN('A', 3)  
#define COL3_PIN PIN('A', 4)  
#define COL2_PIN PIN('A', 5)  
#define COL1_PIN PIN('A', 6)  
#define COL0_PIN PIN('A', 7)  
...
static const uint16_t ROWS[] = {ROW8_PIN, ROW0_PIN, ROW1_PIN, ROW7_PIN, ROW2_PIN, \
                               ROW6_PIN, ROW3_PIN, ROW5_PIN, ROW4_PIN};

static const uint16_t COLS[] = {COL0_PIN, COL1_PIN, COL2_PIN, COL3_PIN, COL4_PIN, \
                               COL5_PIN, COL6_PIN};
...
static inline void gpio_matrix_init(void) {
  for (int m=0; m< NCOLS; m++) {
    gpio_init(COLS[m], GPIO_MODE_INPUT, GPIO_OTYPE_PUSH_PULL, GPIO_SPEED_MEDIUM, GPIO_PULL_DOWN, 0);
  }
  for (int m=0; m< NROWS; m++) {
  gpio_init(ROWS[m], GPIO_MODE_OUTPUT, GPIO_OTYPE_PUSH_PULL, GPIO_SPEED_MEDIUM, GPIO_PULL_UP, 0);
  }
}
...
static inline void set_row(uint16_t val) { // here incorporate the readout of the column and return
